...
fastapi_tables = [t for t in tables if t in ['material_dict', 'boq_item', 'import_batch', 'audit_log', 'match_cache', 'cost_catalog', 'user_favorite', 'tag', 'item_tag']]
print(f'FastAPI 表: {fastapi_tables}')
...
for item in l1_nodes:
    node = MaterialDict(
        level='l1', parent_id=None, name=item['名称'],
        cat_l1=item['名称'], cat_l2=None, cat_l3=None,
...
for item in l2_nodes:
    ...
    node = MaterialDict(
        level='l2', parent_id=parent_id, name=item['名称'],
        cat_l1=code_index.get(parent_code, {}).get('名称', ''),
        cat_l2=item['名称'], cat_l3=None,
        synonyms={'原始编码': item['编码'], '来源': '中建材料字典'},
        spec_whitelist=None,
...
for item in l3_nodes:
    ...
    l2_ancestor = current if current and current['层级'] == 2 else None
    ...
    name_list = [m['名称'] for m in material_names[:50]]

    l1_name = ''
    if l2_ancestor:
        l1_parent = code_index.get(l2_ancestor.get('父编码', ''))
        l1_name = l1_parent.get('名称', '') if l1_parent else ''

    node = MaterialDict(
        level='l3', parent_id=parent_id, name=item['名称'],
        cat_l1=l1_name,
        cat_l2=l2_ancestor['名称'] if l2_ancestor else '',
        cat_l3=item['名称'],
        synonyms={
            '原始编码': item['编码'], '来源': '中建材料字典',
            '材料名称': name_list, '材料名称数': len(material_names),
            '规格叶子数': len(leaves),
        },
...
print('\n抽样验证（前3个l3）:')
for node in db.query(MaterialDict).filter(MaterialDict.level == 'l3').limit(3):
    spec_count = node.synonyms.get('规格叶子数', 0) if node.synonyms else 0
    print(f'  [{node.id}] {node.name} (规格数={spec_count})')
