# -*- coding: utf-8 -*-
"""material_dict —— 五级材料分类字典（架构 §7.3 / M1 §4.6 落地，SQLAlchemy 版）。

设计来源：原 Odoo 版 zaojia.material.dict，后扩展为中建材料字典五级结构。
- 五级树：大类(l1) → 中类(l2) → 小类(l3) → 材料名称(l4) → 规格(l5)
- 读多写少；cat_l1/l2/l3 为冗余平铺（避免递归检索，取前三级祖先）
...
class MaterialDict(Base, BizIdMixin, TimestampMixin):
    ...
    level: Mapped[str] = mapped_column(
        ...
        comment='层级（l1 大类 / l2 中类 / l3 小类 / l4 材料名称 / l5 规格）',
    )
