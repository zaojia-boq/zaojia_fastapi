...
**当前状态**：**M0–M4 核心功能全部完成** + 方案 A 前后端分离 + M2 导入深化四大模块 + 学习引擎 v2/v3 + 体验增强四模块 + 安全加固 + Alembic 版本化迁移 + S8 备份恢复自动化 + **M6 材料字典与清单匹配**（中建材料字典 5 级重构 + 3-2-2-3-4 重编码 + 2024 清单匹配 + 电缆价格聚类测试 V2）+ **M7 单价分析 5 大类正则白名单**（material_classifier.py v2，电缆阻燃归一化/排除误判/加权均价/独立样本明细页）。全量测试 **551 passed**，零失败。
...
- [技术栈](#技术栈)
- [整体架构](#整体架构)
- [核心模块（M0–M4 + M6 + 增强）](#核心模块m0m4--m6--增强)
- [关键设计机制](#关键设计机制)
- [文档索引](#文档索引)
- [里程碑路线图](#里程碑路线图)
- [快速启动](#快速启动)
...
| 当前状态 | M0–M4 ✅ + 前后端分离 ✅ + M2 深化 ✅ + 学习引擎 v2/v3 ✅ + 体验增强 ✅ + 安全加固 ✅ + Alembic ✅ + **M6 材料字典与清单匹配 ✅** |
...
| **M7 单价分析 5 大类正则白名单** | material_classifier.py v2：电缆/钢筋/水泥/混凝土/管道 5 大类正则分类，阻燃归一化，排除配电箱/GRC/灯具/阀门误判，均价按工程量加权，独立样本明细页 | 2655 条清单命中 1837 条（69.2%）；电缆 106 组/1642 条，样本数与明细条数一致 | ✅ 完成 (2026-09-16) |
...
| 文档 | 内容 |
|---|---|
| [`项目总控.md`](项目总控.md) | 项目唯一事实源：目标/架构基线/分工/WBS/评审/变更/ADR/待决事项（v1.9） |
| [`架构设计.md`](架构设计.md) | 总架构、技术栈、数据安全、数据模型、接口契约（v7.2） |
| [`产品设计文档.md`](产品设计文档.md) | 产品通俗说明、场景、功能清单、界面设计语言（v6.4） |
...
*本 README 依据原 Odoo 版设计文档与迁移决策整理，作为 FastAPI 版项目入口概览。最后更新：2026-09-16（M7 单价分析 5 大类正则白名单）。*
